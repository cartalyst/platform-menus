...
<div class="control-group">
	...
	<div class="controls">
		<select{{ $dataChild }} name="{{ sprintf($childName, 'secure') }}" id="{{ $childId }}_secure" class="input-block-level">
			<option value="1"{{ ! empty($child) ? (int) $child->secure : null === 1 ? ' selected="selected"' : null }}>{{ trans('general.yes') }}</option>
			<option value="0"{{ ! empty($child) ? (int) $child->secure : null === 0 ? ' selected="selected"' : null }}>{{ trans('general.no') }}</option>
		</select>
	</div>
</div>
...
<div class="control-group">
	...
	<div class="controls">
		<select{{ $dataChild }} name="{{ sprintf($childName, 'enabled') }}" id="{{ $childId }}_enabled" class="input-block-level">
			<option value="1"{{ ! empty($child) ? (int) $child->enabled : null === 1 ? ' selected="selected"' : null }}>{{ trans('general.enabled') }}</option>
			<option value="0"{{ ! empty($child) ? (int) $child->enabled : null === 0 ? ' selected="selected"' : null }}>{{ trans('general.disabled') }}</option>
		</select>
	</div>
</div>
